...
/**
 * Self Checkout AV Scanner Component
...
export default function SelfCheckout() {
  ...
  async function handleScanSuccess(data) {
    ...
    try {
      if (isOnline) {
        const checkInFunction = httpsCallable(functions, 'checkOut');
        const result = await checkInFunction({
          studentId,
          eventId: urlEventId,
          scannedBy: 'self_checkout_scan'
        });
        ...
        await queueCheckIn({ studentId, eventId: urlEventId, scannedBy: 'self_checkout_scan' });
  ...
  function handleCheckInSuccess(data) {
    showMessage('success', `✓ ${data.studentName} checked out`);
    addToRecentScans({
