...
export default function ServiceLogEntry({
  ...
  if (mode === 'row') {
    return (
      <tr
        className={`text-sm ${!entry.checkOutTime ? 'bg-red-50' : ''} ${entry.forcedCheckoutReason || entry.modificationReason ? 'bg-blue-50' : ''}`}
        data-testid="service-log-row"
      >
        <td className="px-6 py-4">{dateDisplay}</td>
        <td className="px-6 py-4 uppercase font-bold text-[10px] text-blue-600">{activityName}</td>
        <td className="px-6 py-4 text-center text-gray-600">
          {checkInTimeDisplay}
        </td>
        <td className={`px-6 py-4 text-center ${!entry.checkOutTime ? 'text-red-600' : 'text-gray-600'}`}>
          {checkOutTimeDisplay ? (
            checkOutTimeDisplay
          ) : (
        ...
        <td className={`px-6 py-4 text-right font-black ${!entry.checkOutTime ? 'text-red-600' : ''}`}>
          {hoursDisplay}
        </td>
  ...
  return (
    <article
      ...
      aria-label={`Service log entry for ${dateDisplay}`}
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1 min-w-0">
          ...
          <span className="inline-block uppercase font-bold text-[10px] text-blue-600 mt-1 bg-blue-50 px-2 py-0.5 rounded">
            {activityName}
          </span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 text-sm mb-4">
        ...
        <div>
          ...
          <span className={checkOutTimeDisplay ? 'text-gray-900 font-medium' : 'text-red-600 font-medium text-xs'}>
            {checkOutTimeDisplay || 'Not checked out'}
          </span>
        </div>
        <div>
          <span className="block text-xs text-gray-500 uppercase font-medium mb-1">Hours</span>
          <span className={`font-black ${!entry.checkOutTime ? 'text-red-600' : 'text-gray-900'}`}>
            {hoursDisplay}
          </span>
        </div>
      </div>
